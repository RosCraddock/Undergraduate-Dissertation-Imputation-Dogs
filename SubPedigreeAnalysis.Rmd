...
```{r}
AF_Sub_Data <- read.csv("/Users/roscraddock/Documents/University\ of\ Edinburgh/HighlanderLab - AlleleFetch_data/Clean Pedigrees/PBGV/PBGV_Sub_2009-2014.csv")
AF_Sub_Result <- read.csv("/Users/roscraddock/Documents/University of Edinburgh/HighlanderLab - AlleleFetch_data/Clean Pedigrees/PBGV/AlleleFetch Inputs/PBGV_ResultPheno_2009-2014.csv")
AF_Sub_Result <- AF_Sub_Result %>%
  select(!X)
...
# Plotting the Mean frequency of states per year (2009 to January 2014)
AFPheno <- AF_Sub_Trend_Plot %>% ggplot(aes(year, value, group=State, col=State))+
  geom_line(size=1) +
  xlab("Year of Birth") +
  ylab("Mean Frequency") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0.5)) +
...
AF_Sub_ResultAge <- AF_Sub_ResultAge %>%
  pivot_wider(names_from = c(State, Var), 
              values_from = Freq)
#Merge together
AF_Sub_Trend <- merge(AF_Sub_ResultAge, AF_Sub_Data, by.x = "IID", by.y = "IRN")
...
# Plotting the Mean frequency of states per year (2009 to January 2014)
AFPhenoAge <- AF_Sub_Trend_Plot %>% ggplot(aes(year, value, group=State, col=State))+
  geom_line(size=1) +
  xlab("Year of Birth") +
  ylab("Mean Frequency") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0.5)) +
